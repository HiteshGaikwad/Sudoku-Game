...
public class Board{
    ...
    //Each AL will take 3 different Arrays
    ...
    private void createBoard(){
      ...
      constructLinesFromAl(second,3);
      constructLinesFromAl(third,6);
    }
    ...
    private ArrayList Collectionshuffle(ArrayList<ArrayList<int[]>> al){
     for(int i=0; i<al.size(); i++){
      //fisher-yetes approach of shuffling
    ...
    private void initializeThreeAl(int [] lineOne, int[] lineTwo, int [] lineThree, int[] lineFour, int[] lineFive, int [] lineSix, int[] lineSeven, int[] lineEight, int[] lineNine){
     first.clear();
     second.clear();
     third.clear();
    ...
    private void hideBoardValue(){
     //algorith to hide values
         for(int row=0; row<9; row++){
             int zeroToMake = (int) (Math.random()*3)+3;
             double temp;
             double removeZeroToMake;

             for(int col=0; col<9; col++) {
              temp = Math.random();
              removeZeroToMake = (double) zeroToMake / (double) (9 - col);

              if(removeZeroToMake <= temp) {
               zeroToMake--;
